...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr  5 23:17:23 2025
...
from TT2VQC import MPSParamVQC 
...
parser.add_argument('--num_qubits', default=12, help='Number of qubits in the quantum circuit', type=int)
...
if __name__ == "__main__":
    ...
    model = MPSParamVQC(
        ...
        n_wires=num_qubits,
        n_qlayers=depths,
        tensor_product_enc=False,  # or True, depending on your data
        add_fc=True,
        out_features=2,
        noise_prob=0.1
    ).to(device)
    ...
    # Count only the parameters that will actually be trained (i.e. requires_grad=True).
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Number of trainable parameters (excluding frozen backbone): {trainable_params}")

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=lr)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.1) # For each 10 iterations, the lr is divided by 2
